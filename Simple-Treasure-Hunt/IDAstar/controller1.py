...
def getIndex(l, tup):
	...
	for i in range(len(l)):
		...
		return -1;
def aStar(board, loc, exploredNodes, searchQueue, shortestPath, lim):
	...
	exceedLim = 100**100;
	flag = 0;
	if(g+heuristic(loc, diamondLoc) <= lim):
		h.addNode(loc, g, heuristic(loc, diamondLoc));
	elif(g+heuristic(loc, diamondLoc) < exceedLim):
		exceedLim = g+ heuristic(loc, diamondLoc);
	while(h.currentSize > 0):
		...
		if(board[x][y] == 3):
			flag = 1;
			if(searchQueue[iteration] == []):
				del searchQueue[iteration];
			exceedLim = -1;
			break;
		if(y-1 >= 0 and (board[x][y-1] == 2 or board[x][y-1] == 3) and gValue +1+ heuristic([x,y-1], diamondLoc) <= lim and colorMat[x][y-1] == WHITE):
			colorMat[x][y-1] = GRAY;
			...
			if(y-1 >= 0 and (board[x][y-1] == 2 or board[x][y-1] == 3) and colorMat[x][y-1] == WHITE and exceedLim > gValue + 1 + heuristic([x,y-1], diamondLoc)):
				exceedLim = gValue + 1 + heuristic([x,y-1], diamondLoc);
		if(x+1 < len(board) and (board[x+1][y] == 2 or board[x+1][y] == 3) and 1 + gValue + heuristic([x+1,y], diamondLoc) <= lim and colorMat[x+1][y] == WHITE):
			colorMat[x+1][y] = GRAY;
			...
			if(exceedLim > gValue + 1 + heuristic([x+1,y], diamondLoc) and x+1 < len(board) and (board[x+1][y] == 2 or board[x+1][y] == 3) and colorMat[x+1][y] == WHITE):
				exceedLim = gValue + 1 + heuristic([x+1,y], diamondLoc);
		if(y+1 < len(board[x]) and (board[x][y+1] == 2 or board[x][y+1] == 3) and gValue + 1 + heuristic([x,y+1], diamondLoc) <= lim and colorMat[x][y+1] == WHITE):
			colorMat[x][y+1] = GRAY;
			l = heuristic([x,y+1], diamondLoc);
			h.addNode([x,y+1], gValue + 1, l);						
			searchQueue[iteration].append([x, y+1]);
			...
			if(y+1 < len(board[x]) and (board[x][y+1] == 2 or board[x][y+1] == 3) and exceedLim > gValue + 1 + heuristic([x,y+1], diamondLoc) and colorMat[x][y+1] == WHITE):
				exceedLim = gValue + 1 + heuristic([x,y+1], diamondLoc);
		if(x-1 >= 0 and (board[x-1][y] == 2 or board[x-1][y] == 3) and 1 + gValue + heuristic([x-1,y], diamondLoc) <= lim and colorMat[x-1][y] == WHITE):
			colorMat[x-1][y] = GRAY;
			l = heuristic([x-1,y], diamondLoc);
			h.addNode([x-1,y], gValue + 1, l);			
			searchQueue[iteration].append([x-1, y]);
			...
			if(x-1 >= 0 and (board[x-1][y] == 2 or board[x-1][y] == 3) and exceedLim > 1 + gValue + heuristic([x-1,y], diamondLoc) and colorMat[x-1][y] == WHITE):
				exceedLim = gValue + 1 + heuristic([x-1,y], diamondLoc);
		colorMat[x][y] = BLACK;
...
def singleAgentSearch(board):
	...
	for i in range(len(loc)):
		while(lim != -1 and lim != 100**100):
			exploredNodes = [];
			
			shortestPath = [];
			lim = aStar(board, loc[i], exploredNodes, searchQueue, shortestPath, lim);
