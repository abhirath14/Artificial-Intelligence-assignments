...
class Heap:
	currentSize = 0;
	heapList = [];
	def addNode(self, pair, distance):
		# adds a node to the heap list
		self.heapList.append([pair, distance]);
		self.currentSize+=1;
		index = self.currentSize-1;
		while(self.heapList[parent(index)][1] > self.heapList[index][1]):
	...
	def minHeapify(self, index):
		minimum = index;
		if(rChild(index) < self.currentSize and self.heapList[rChild(index)][1] < self.heapList[minimum][1]):
			minimum = rChild(index);
		if(lChild(index) < self.currentSize and self.heapList[lChild(index)][1] < self.heapList[minimum][1]):
...
def getIndex(l, tup):
	...
	for i in range(len(l)):
		...
		return -1;
def bestFirstSearch(board, loc, exploredNodes, searchQueue, shortestPath):
	...
	h.clearAll();
	flag = 0;
	h.addNode(loc, heuristic(loc, diamondLoc));
	while(h.currentSize > 0):
		[[x,y],value] = h.extractMin();
...
def singleAgentSearch(board):
	...
	for i in range(len(loc)):
		...
		searchQueue = [];
		shortestPath = [];
		bestFirstSearch(board, loc[i], exploredNodes, searchQueue, shortestPath);
